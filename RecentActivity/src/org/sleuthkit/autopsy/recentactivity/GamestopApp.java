...
/**
 * Gamestop Digital Distribution Client Browser recent activity extraction
 */
public class GamestopApp extends Extract {

    private static final String gamestopclientcookiequery = "select name, value, host_key, expires_utc,last_access_utc, creation_utc from cookies";
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    public int GamestopAppBrowserCount = 0;
    final public static String MODULE_VERSION = "1.0";
    private IngestServices services;

    //hide public constructor to prevent from instantiation by ingest module loader
    GamestopApp() {
        moduleName = "Gamestop Digital Distribution Platform";
    }
    ...
    private void getCookie(Content dataSource, IngestDataSourceWorkerController controller) {
        ...
        try {
            cookiesFiles = fileManager.findFiles(dataSource, "Cookies", "Gamestop/App/cache");
        } catch (TskCoreException ex) {
            logger.log(Level.SEVERE, "Error when trying to get Gamestop digital distribution client browser cookie files.", ex);
        }
        ...
        if (cookiesFiles != null && !cookiesFiles.isEmpty()) {
            while (j < cookiesFiles.size()) {
                ...
                try {
                    ...
                    logger.log(Level.SEVERE, "Error writing temp sqlite db for Gamestop digital distribution client browser cookie artifacts.{0}", ex);
                ...
                List<HashMap<String, Object>> tempList = this.dbConnect(temps, gamestopclientcookiequery);
                ...
                for (HashMap<String, Object> result : tempList) {
                    ...
                    bbattributes.add(new BlackboardAttribute(ATTRIBUTE_TYPE.TSK_PROG_NAME.getTypeID(), "Recent Activity", "Gamestop digital distribution client browser"));
                ...
                if (errors > 0) {
                    this.addErrorMessage(this.getName() + ": Error parsing " + errors + " Gamestop digital distribution client browser cookie artifacts.");
                }
    ...
    public void complete() {
        logger.info("Gamestop Digital Distribution Client Browser Extract has completed");
    }

    @Override
    public void stop() {
        logger.info("Attempted to stop Gamestop Digital Distribution Client browser extract, but operation is not supported; skipping...");
    }

    @Override
    public String getDescription() {
        return "Extracts activity from the Gamestop Digital Distribution client browser.";
    }
