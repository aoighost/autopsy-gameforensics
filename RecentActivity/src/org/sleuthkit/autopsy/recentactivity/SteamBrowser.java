 ...
 */
public class SteamBrowser extends Extract {

    private static final String steambrowseroverlaycookiequery = "select name, value, host_key, expires_utc,last_access_utc, creation_utc from cookies";
    private static final String steambrowserclientcookiequery = "select name, value, host_key, expires_utc,last_access_utc, creation_utc from cookies";
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    public int SteamBrowserCount = 0;
    final public static String MODULE_VERSION = "1.0";
    private IngestServices services;

    //hide public constructor to prevent from instantiation by ingest module loader
    SteamBrowser() {
        moduleName = "Steam Browser";
    }
    ...
    public void process(PipelineContext<IngestModuleDataSource>pipelineContext, Content dataSource, IngestDataSourceWorkerController controller) {
        this.getClientCookie(dataSource, controller);
        this.getOverlayCookie(dataSource, controller);
    }

    //COOKIES section
    // This gets the cookie info
    private void getClientCookie(Content dataSource, IngestDataSourceWorkerController controller) {
        
        FileManager fileManager = currentCase.getServices().getFileManager();
        List<AbstractFile> cookiesFiles = null;
        try {
            cookiesFiles = fileManager.findFiles(dataSource, "Cookies", "config/htmlcache");
        } catch (TskCoreException ex) {
            logger.log(Level.SEVERE, "Error when trying to get Steam client browser cookie files.", ex);
        }
        ...
        if (cookiesFiles != null && !cookiesFiles.isEmpty()) {
            while (j < cookiesFiles.size()) {
                ...
                try {
                    ...
                    logger.log(Level.SEVERE, "Error writing temp sqlite db for Steam client browser cookie artifacts.{0}", ex);
                ...
                List<HashMap<String, Object>> tempList = this.dbConnect(temps, steambrowserclientcookiequery);
                ...
                for (HashMap<String, Object> result : tempList) {
                    ...
                    bbattributes.add(new BlackboardAttribute(ATTRIBUTE_TYPE.TSK_PROG_NAME.getTypeID(), "Recent Activity", "Steam client browser"));
                ...
                if (errors > 0) {
                    this.addErrorMessage(this.getName() + ": Error parsing " + errors + " Steam client browser cookie artifacts.");
                }
    ...
    private void getOverlayCookie(Content dataSource, IngestDataSourceWorkerController controller) {
        
        FileManager fileManager = currentCase.getServices().getFileManager();
        List<AbstractFile> cookiesFiles = null;
        try {
            cookiesFiles = fileManager.findFiles(dataSource, "Cookies", "config/overlayhtmlcache");
        } catch (TskCoreException ex) {
            logger.log(Level.SEVERE, "Error when trying to get Steam in-game overlay browser cookie files.", ex);
        }
        ...
        if (cookiesFiles != null && !cookiesFiles.isEmpty()) {
            while (j < cookiesFiles.size()) {
                ...
                try {
                    ...
                    logger.log(Level.SEVERE, "Error writing temp sqlite db for Steam in-game overlay browser cookie artifacts.{0}", ex);
                    this.addErrorMessage(this.getName() + ": Error while trying to analyze file:" + cookiesFile.getName());
                }
                File dbFile = new File(temps);
                if (controller.isCancelled()) {
                ...
                for (HashMap<String, Object> result : tempList) {
                    ...
                    bbattributes.add(new BlackboardAttribute(ATTRIBUTE_TYPE.TSK_DATETIME.getTypeID(), "Recent Activity", (((Long.valueOf(result.get("last_access_utc").toString())) / 1000000) - 11644473600L)));
                    bbattributes.add(new BlackboardAttribute(ATTRIBUTE_TYPE.TSK_VALUE.getTypeID(), "Recent Activity", ((result.get("value").toString() != null) ? result.get("value").toString() : "")));
                    bbattributes.add(new BlackboardAttribute(ATTRIBUTE_TYPE.TSK_PROG_NAME.getTypeID(), "Recent Activity", "Steam in-game overlay browser"));
                    bbattributes.add(new BlackboardAttribute(ATTRIBUTE_TYPE.TSK_URL.getTypeID(), "Recent Activity", ((result.get("host_key").toString() != null) ? result.get("host_key").toString() : "")));
                    bbattributes.add(new BlackboardAttribute(ATTRIBUTE_TYPE.TSK_URL_DECODED.getTypeID(), "Recent Activity", ((result.get("host_key").toString() != null) ? EscapeUtil.decodeURL(result.get("host_key").toString()) : "")));
                    String domain = result.get("host_key").toString();
                    domain = domain.replaceFirst("^\\.+(?!$)", "");
                ...
                if (errors > 0) {
                    this.addErrorMessage(this.getName() + ": Error parsing " + errors + " Steam in-game overlay browser cookie artifacts.");
                }
    ...
    public void complete() {
        logger.info("Steam Browser Extract has completed");
    }

    @Override
    public void stop() {
        logger.info("Attempted to stop Steam browser extract, but operation is not supported; skipping...");
    }

    @Override
    public String getDescription() {
        return "Extracts activity from the Steam client and in-game overlay browsers.";
    }
